...
import { referralApi, ReferralDashboard, ReferredUser, ReferralEarning } from '@/lib/api'
...
export default function ReferralsPage() {
  ...
  async function loadData() {
    try {
      const [dash, users, earns] = await Promise.all([
    ...
    <div className="p-6 space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">
          Запрошуйте косметологів та отримуйте {dashboard.commission_pct}% від їх підписок
        </p>
      </div>
            ...
            {referredUsers.length === 0 ? (
              <Card>
                <CardContent className="py-12 text-center">
                  ...
                  <p className="text-muted-foreground">
                    Ви ще не запросили нікого. Поділіться посиланням з косметологами!
                  </p>
                </CardContent>
              </Card>
            ) : (
